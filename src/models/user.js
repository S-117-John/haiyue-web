...
export default {
  ...
  effects: {
    ...
    *fetchPlatformDetail({ payload }, { call, put }) {
      const response = yield call(platformDetail, payload);
      if (response.success) {
        yield put({
          type: 'saveDetail',
          payload: {
    ...
    *updatePlatform({ payload }, { call }) {
      const response = yield call(updatePlatform, payload);
      if (response.success) {
        message.success('提交成功');
        router.push('/system/user');
      }
