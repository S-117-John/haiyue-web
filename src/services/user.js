...
import { captchaMode, switchMode } from '../defaultSettings';
...
export async function accountLogin(params) {
  ...
  return request('/api/blade-auth/oauth/token', {
    headers: {
      ...
      'Dept-Id': switchMode ? values.deptId : '',
      'Role-Id': switchMode ? values.roleId : '',
...
export async function updatePlatform(params) {
  return request('/api/blade-user/update-platform', {
    method: 'POST',
    body: func.toFormData(params),
  });
}
...
export async function platformDetail(params) {
  return request(`/api/blade-user/platform-detail?${stringify(params)}`);
}
